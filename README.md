...
* Pull the .proto files information out of the codebase
* Auto generate code blocks from data in the proto file
	* May need to get creative with js or something to insert the code in the correct locations




## Auto Building API Instructions

This is the old setup and is how I created the docs shown at http://api.fr1t2.com/api

The scripts need help and the entire process becomes not automated when we enter code blocks.

__Basic build steps are:__
- update the qrl repo grabbing the latest proto files
- run the docs generator found https://github.com/pseudomuto/protoc-gen-doc on teh .proto file generating a .md file
- command line script fu to pull out any unnessasary lines and format to fit the slate structure (side menu links and headers mostly)
- insert the correct front matter from this repo into the head of the document
- This is then moved to the webroot, and is missing any code blocks.

__Where this breaks__
- once we include code blocks we have an issue, and now must manually enter informtion. 
   - the code blocks should live at the beginning of any section, telling the slate docs site to format and insert into the code section. 
   - we need to insert a block of code for each language we intend to serve examples for.
- I have entered the information manually from the integration tests into some sections for javascript and that is what is serverd. 
- This breaks the automated updates for the site, which isnt the worlds end, but makes upkeep difficult.


---

The following instructions also include hosting the jekyll site for the docs. Disreguard any jekyll setup instructions for now. this was my cheatsheet for setup.
